...
### Running the program :
- Build the program for your machine : `go build -race`. It will install all the necessary modules

   > The `-race` flag is there to ensure that any data race is detected, but there aren't any.
- Create an input file following the example of `websites.txt`
- Run the binary : `./websiteMonitor -f input_filename`
...
- I chose to use a third-party module,
  ...
  terminal interface for the logs and alert. The repository has around 7k stars on
  Github, and the code is easily readable, although its last version is from 2021.
